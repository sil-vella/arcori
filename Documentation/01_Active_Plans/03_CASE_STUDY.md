...
1. **P0** Celebration / mastery anims + Achievements env `016` (Daily Missions + Daily Cache claim shipped) — [core-match-loop.md](core-match-loop.md) · [daily-goals.md](daily-goals.md) · [achievements.md](achievements.md)
...
| Disc face from catalog | Flipped Arcori should look like the design, with its rim color; art ready before flip | Freeze stamps `imageUrl`/`color` (and `lottieUrl` when present) on `table.pieces`; Flutter `ArcoriCylinder` SSOT (match, Avari, Velora browse/Detail); Play precaches circulating art; face-down still decodes `Image.network` |
| Lottie face for any theme | Kin is not the only Lottie face — catalog may stamp `faceMedia: lottie` + `lottieUrl` on regular Arcori | Index/get_design/theme pass through face fields; Kin still resolves via `/media/kin/players/` + `art_basename` |
...
| Daily Cache = fixed +2 Gold Fragments | Simple claim loot without weighted mystery tables for v1 | `claim_gate` id `daily_mystery_box`; JSON `reward.amount`; `apply_fragment_delta`; `daily_cache_claimed_at`; Tasks Claim + soft returning nudge — [daily-goals.md](daily-goals.md) |
